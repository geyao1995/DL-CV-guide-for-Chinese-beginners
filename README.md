...
在进行深度学习实验时，需要调整超参数设置的来观察训练的效果。如何将不同结果和参数配置对应起来是一个值得考虑的问题，简单地，这里举例两种常见的做法：

1. 通过文件夹的名称表示文件夹中的数据是由哪些超参数产生的。比如，将文件夹命名为：`lr-0.1_epoch-15_bs-128_model-3-layer_...`
2. 将超参数保存为一个单独的文件，比如 `json` 格式。

这两种做法虽然能保存超参数，但不能直观的结合实验数据进行对比（比如达到下面的对比效果）：

![result_compare_1](./imgs/result_compare_1.png)

![result_compare_1](./imgs/result_compare_2.png)

借助 [W&B](https://github.com/wandb/wandb) 工具，可以更好的对实验进行管理，上面两幅图也是由 W&B 生成。

请参考 [W&B 官方文档](https://docs.wandb.ai/)，使用 W&B 完成以下几点：

- 如何保存超参数？
- 如何记录指标？
- 如何保存每一次实验的代码？
- 如何在 W&B 网页端 (UI) 查看/对比实验？
- 如何将 W&B 保存的指标数据加载为 numpy 格式数组？
- 如何进行超参数搜索 (parameters sweep)？

**参考代码**：[wandb_demo](https://github.com/geyao1995/wandb_demo)
